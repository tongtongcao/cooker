...
Long_t Det_MWPC::process_event(){
  ...
  if(treeRaw->event<0){
    for(int i=0;i<12;i++){
      ...
      event_pedSub[i]->addHistData_pedSub(data[i],pedValue[i],pedSigma[i]);
    ...
    legend->AddEntry(event[0]->getHist(),"Ped Sup","l");
    legend->AddEntry(event_pedSub[0]->getHist(),"+ Ped Sub","l");
  ...
  for(int i=0;i<12;i++){
    ...
    adcValue[i]=event[i]->getAdcValue(data[i],pedValue[i],pedSigma[i],0);
