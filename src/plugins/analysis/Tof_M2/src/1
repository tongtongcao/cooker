...
Long_t Tof_M2::histos(){
  ...
  final_PgCAdc =  new TH1F( "final_PgCAdc","final_PgCAdc",80,0,adc_c);
  final_PgCEp =  new TH1F( "final_PgCEp","final_PgCEp",50,0,Ep_c);
  ...
  vert_Mom =  new TH1F( "vert_Mom","vert_Mom",150,150,280);
  ...
  for(int i = 0; i < 12; i++){
    ...
    h1EP_PgC[i] = dH1(name1kk.c_str(),title1kk.c_str(),25,0,Ep_c);
    ...
    hBeta_gap[i] = dH2(Form("Beta_Gap_%d ",i+1),Form("Beta_Gap_%d ",i+1),12,0,12,70,0,3);
    hBeta_gap[i]->GetXaxis()->SetTitle("Gap");
    hBeta_gap[i]->GetXaxis()->CenterTitle();
    hBeta_gap[i]->GetYaxis()->SetTitle("#beta");
    hBeta_gap[i]->GetYaxis()->CenterTitle();
    ...
    t1+=(i+1)%10+'0';
    t1+="Mom";
    h1Mom_ADC[i] = dH2(n1.c_str(),t1.c_str(),100,0,1000,100,0,400);
    ...
    tpt1+=(i+1)%10+'0';
    tpt1+="Momtp";
    h1Mom_ADC_TP[i] = new TProfile(tpn1.c_str(),tpt1.c_str(),100,0,500,0,400);
    ...
    for(int ii=0;ii<7;ii++){
      ...
      ADC_PgCh[i][ii]= dH1(namep.c_str(),titlep.c_str(),15,0,adc_c);
      ...
      titlep11+=(ii)+'0';
      titlep11+="";
      EP_PgCh[i][ii]= dH1(namep11.c_str(),titlep11.c_str(),15,0,Ep_c);
...
Long_t Tof_M2::startup()
{
  ...
  string inputfile_C =  Form("/data/trek/E36/gautam/crnRoot/cooker/inputFiles/tofM2/pgc_gains/PgC_Gain_align_peak_ke3eP.dat");
...
Long_t Tof_M2::process()
{
  ...
  Ep_PgCf=0;
  //239.61756  231.28756
  if(ptr->nTracks==1 &&  ptr->fChi2PerNDF<10 && 
//      ptr->fVertSP<239.61756 && ptr->fVertSP>231.28756 && 
     ptr->fVertSP<225 &&  
    ...
    betaGap[ptr->fgapNumTof2] += sum_beta_A[ptr->fgapNumTof2]+sum_beta_B[ptr->fgapNumTof2]; 
    F_beta += betaGap[ptr->fgapNumTof2];
    hBeta_gap[ptr->fgapNumTof2]->Fill(ptr->fgapNumTof2, betaGap[ptr->fgapNumTof2]);
	...
	for(int i=0;i<12;i++){
	  ...
	  if(Array_ADC[i][0]>cut_val && Array_ADC[i][1]<cut_val && Array_ADC[i][2]<cut_val && Array_ADC[i][3]<cut_val && Array_ADC[i][4]<cut_val && Array_ADC[i][5]<cut_val && Array_ADC[i][6]<cut_val)  
	    {
	      ...
	      EP_PgCh[i][0]->Fill(array_ADC[i][0]/P_PgC[i]);
	  ...
	  if(i==2 && ADC_PgC[2]>30){
	    ...
	    ADC_PgCf += ADC_PgC[i]; //
	    Ep_PgCf += ADC_PgC[i]/tof2_mom[i];	  	 
	 
	  }
	} //end of i loop
       	if(ADC_PgCf>6)final_PgCAdc->Fill(ADC_PgCf);
	if(Ep_PgCf>0.1)final_PgCEp->Fill(Ep_PgCf);
...
Long_t Tof_M2::done()
{
  ...
  cEp1->Divide(3,4); 
  TCanvas* cBG = new TCanvas("cBG", " Beta vs Gap", 100, 100, 800, 650);
  cBG->Divide(3,4);
  ...
  TLine *line = new TLine(4200,0,4200,max_M2);
  ...
  /*c3->cd(7);
  theta_pzx->Draw("colz");
  theta_pzx->GetXaxis()->SetTitle("#theta(rad)");
  theta_pzx->GetXaxis()->CenterTitle();
  theta_pzx->GetYaxis()->SetTitle("Path length for z-x projection");
  theta_pzx->GetYaxis()->CenterTitle();
  c3->cd(8);
  phi_pzx->Draw("colz");
  phi_pzx->GetXaxis()->SetTitle("#phi(rad)");
  phi_pzx->GetXaxis()->CenterTitle();
  phi_pzx->GetYaxis()->SetTitle("Path length for z-x projection");
  phi_pzx->GetYaxis()->CenterTitle();
  c3->cd(9);
  p_pzx->Draw("colz");
  p_pzx->GetXaxis()->SetTitle("Momentum at c4 (MeV)");
  p_pzx->GetXaxis()->CenterTitle();
  p_pzx->GetYaxis()->SetTitle("Path length for z-x projection");
  p_pzx->GetYaxis()->CenterTitle();
 */
  TCanvas *c11[12];
  TCanvas *c12[12];
  for(int i = 0; i < 12; i++){
    ...
    /* if(i==4){
       h1Mom_ADC_TP[2]->Fit("pol1","","",0,350);
       }*/
    //else if (i!=4){
    h1Mom_ADC_TP[i]->Fit("pol1","","",0,350);

    //}
    h1Mom_ADC_TP[i]->GetYaxis()->SetRangeUser(210,230);
  ...
  c3->SaveAs("/data/trek/E36/gautam/crnRoot/cooker/plots/PgC_kmu2PR.pdf"); 
  c2->Write("AC_adcDsub");
  c3->Write("final_var");
