...
TF1* f1=mH1->GetFunction("gaus");
  if(f1==0){
    ...
    if(nfound == 1){
      ...
      lowRange=mH1->GetBinLowEdge(mH1->GetMaximumBin()-3);  //xpeaks[0]/2.0;
      upRange=mH1->GetBinLowEdge(mH1->GetMaximumBin()+3);   //xpeaks[0]*3.0/2.0;
      ...
      Float_t *xpeaks = s->GetPositionX();
      std::sort(xpeaks,xpeaks+nfound);
      lowRange=mH1->GetBinLowEdge(mH1->GetMaximumBin()-3);
      upRange=mH1->GetBinLowEdge(mH1->GetMaximumBin()+3);   //xpeaks[0]*3.0/2.0;
      mH1->Fit("gaus","Q","",lowRange,upRange);
      ...
      Float_t *xpeaks = s->GetPositionX();
      std::sort(xpeaks,xpeaks+nfound);
      lowRange=mH1->GetBinLowEdge(mH1->GetMaximumBin()-4);  //xpeaks[0]/2.0;
      upRange=mH1->GetBinLowEdge(mH1->GetMaximumBin()+5);   //xpeaks[0]*3.0/2.0;
      mH1->Fit("gaus","Q","",lowRange,upRange);
