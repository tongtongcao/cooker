...
int adc_c = 400;
int Ep_c = 4;
  final_PgCAdc =  new TH1F( "final_PgCAdc","final_PgCAdc",30,2,600);
  ...
  for(int i = 0; i < 12; i++){
    ...
    h1adc_PgC[i] = dH1(name1.c_str(),title1.c_str(),40,10,adc_c);
    ...
    h1EP_PgC[i] = dH1(name1kk.c_str(),title1kk.c_str(),30,0,Ep_c);
  ...
  if(ptr->nTracks==1 &&  ptr->fVertSP <300. && 
	...
	for(int i=0;i<12;i++){
	  ...
	  for(int ii=0;ii<7;ii++){
	   ...
	   else if( i==2 && ii==6) Array_ADC[i][ii]=0;        
