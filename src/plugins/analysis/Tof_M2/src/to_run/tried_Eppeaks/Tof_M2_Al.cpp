...
int adc_c = 400;
int Ep_c = 4;
  final_PgCAdc =  new TH1F( "final_PgCAdc","final_PgCAdc",30,0.1,600);
  final_PgCEp =  new TH1F( "final_PgCEp","final_PgCEp",22,0,4);
...
if(par=="ke3") M = 0.511;
  ...
  string inputfile_C = Form("/data/trek/E36/gautam/crnRoot/cooker/inputFiles/tofM2/pgc_gains/PgC_Gain_align_peak_%seP.dat",par.Data());
...
Long_t Tof_M2::process()
{
  ...
  ADC_PgCf=0;
   Ep_PgCf=0;
  if(ptr->nTracks==1 &&  ptr->fVertSP <230. && 
...
/*else if(Array_ADC[2][6]>200) {
	       sf = sf_m/peak_val[2][6]; 
               array_ADC[2][6]= Array_ADC[2][6];
	       EP_PgCh[2][6]->Fill(array_ADC[2][6]/tof2_mom[2]);
	      ADC_PgCh[2][6]->Fill(array_ADC[2][6]);
	      ADC_PgC[2] +=array_ADC[2][6];   
}

else if(Array_ADC[7][5]>150) {
	       sf = sf_m/peak_val[7][5]; 
               array_ADC[7][5]= Array_ADC[7][5];
	       EP_PgCh[7][5]->Fill(array_ADC[7][5]/tof2_mom[7]);
	      ADC_PgCh[7][5]->Fill(array_ADC[7][5]);
	      ADC_PgC[7] +=array_ADC[7][5];   
}
...
/*  c3->cd(7);
  ...
  theta_pzx->GetYaxis()->SetTitle("Path length for z-x projection of trajectories");
  theta_pzx->GetYaxis()->CenterTitle();
  c3->cd(8);
  phi_pzx->Draw("colz");
  phi_pzx->GetXaxis()->SetTitle("#phi(rad)");
  phi_pzx->GetXaxis()->CenterTitle();
  phi_pzx->GetYaxis()->SetTitle("Path length for z-x projection of trajectories");
  phi_pzx->GetYaxis()->CenterTitle();
  c3->cd(9);
  p_pzx->Draw("colz");
  p_pzx->GetXaxis()->SetTitle("Momentum at c4 (MeV)");
  p_pzx->GetXaxis()->CenterTitle();
  p_pzx->GetYaxis()->SetTitle("Path length for z-x projection of trajectories");
...
c3->SaveAs("/data/trek/E36/gautam/crnRoot/cooker/plots/PgC_ke3LM.pdf");  
  c2->Write("AC_adcDsub");
  c3->Write("final_var");
