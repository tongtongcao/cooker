...
void fieldStepper::transport(trackState *svFrom, planeHit *hitTo, matrix &sv, matrix &F, matrix &Q, particleId particleId, Double_t particleMass, Double_t particleCharge, Bool_t &magFlag, matType type){
  ...
  if(magFlag == kTRUE){
    ...
    propagateStraightLine(svTo, DF, hitTo, particleMass, particleCharge);
...
Double_t fieldStepper::calcEnergyLoss(matrix &Q, matrix &svTo, Double_t length, Double_t qp, Double_t beta, particleId particleId, Double_t particleMass, Double_t particleCharge, matType type)
{
  ...
  Double_t elossIon = 0.;
   ...
   if(particleId == kPositronId) {
    // Radiation loss for positrons.
     elossRad = calcRadLossPositron(length, qp, fDetMatProperties[type][kRadLength]);
      if(fIsBackward) {
	elossRad *= -1.;
      }
  ...
  //Double_t eloss = elossIon+elossRad; // delta(E)
...
Double_t fieldStepper::calcRadLossPositron(Double_t length, Double_t qp, Double_t radLength)
{
  ...
  Double_t me = 0.510998918; // positron mass
  Double_t elossRad = (TMath::Exp(-t) - 1) * sqrt(1/qp * 1/qp + me * me);
