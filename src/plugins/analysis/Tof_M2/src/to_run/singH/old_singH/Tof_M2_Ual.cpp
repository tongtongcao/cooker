...
int Ep_uc = 12;
int Ep_c = 6;
 final_PgCAdc =  new TH1F( "final_PgCAdc","final_PgCAdc",20,10,adc_uc);
  ...
  for(int i = 0; i < 12; i++){
    ...
    h1adc_PgC[i] = dH1(name1.c_str(),title1.c_str(),10,0,adc_uc);
    ...
    h1EP_PgC[i] = dH1(name1kk.c_str(),title1kk.c_str(),30,0,Ep_uc);
    ...
    for(int ii=0;ii<7;ii++){
      ...
      ADC_PgCh[i][ii]= dH1(namep.c_str(),titlep.c_str(),10,10,adc_uc);
      ...
      titlep11+=(ii)+'0';
      titlep11+="";
      EP_PgCh[i][ii]= dH1(namep11.c_str(),titlep11.c_str(),12,0,Ep_uc);
...
int cut_val = 70;
	...
	for(int i=0;i<12;i++){
	  ...
	  for(int ii=0;ii<7;ii++){
       ...
       if(Array_ADC[i][ii]>70){ 
             count_j++;
           //    }
    if(count_j==1){       

              array_ADC[i][ii]= Array_ADC[i][ii]*sf;
	      EP_PgCh[i][ii]->Fill(array_ADC[i][ii]/tof2_mom[i]);
	      ADC_PgCh[i][ii]->Fill(array_ADC[i][ii]);
	      ADC_PgC[i] +=array_ADC[i][ii];   

        //      cout<<" count:  "<<count_j<<" sun_count  "<<sum_count<<endl;
         }

               if(count_j==1) sum_count += count_j;
               if(count_j==2) sum_count2 += count_j;
               if(count_j==3) sum_count3 += count_j;
               if(count_j==4) sum_count4 += count_j;
		
cout<<" count1:  "<<sum_count<<"  count2:  "<<sum_count2<<"  count3:  "<<sum_count3<<"  count4:  "<<sum_count4<<endl;

	    }

}

	  if(ADC_PgC[i]/tof2_mom[i]/1.5>0.3)h1EP_PgC[i]->Fill(ADC_PgC[i]/tof2_mom[i]);
...
Long_t Tof_M2::done()
{
  ...
  //FILE *mean_B1 = fopen("/data/trek/E36/gautam/crnRoot/cooker/inputFiles/tofM2/pgc_gains/PgC_Gain_align_Check.dat","w");
  FILE *mean_B1 = fopen("/data/trek/E36/gautam/crnRoot/cooker/inputFiles/tofM2/pgc_gains/PgC_Gain_align_peak.dat","w");
  ...
  for(int i = 0; i < 12; i++){
    ...
    for(int ii = 0; ii < 7; ii++){
      ...
      //ADC_PgCh[i][ii]->Fit("gaus");
      //  TF1 *fit = ADC_PgCh[i][ii]->GetFunction("gaus");
      // Mean_B[i][ii] = ADC_PgCh[i][ii]->GetMean();
           Max_B[i][ii] =  ADC_PgCh[i][ii]->GetBinCenter(ADC_PgCh[i][ii]->GetMaximumBin());;
         fprintf(mean_B1,"%d      %d     %1.4f\n",i,ii, Max_B[i][ii]);
      //   gPad->SetLogy();  
      // cout<<"  i  "<<i<<"  ii   "<<ii<<"  Max:  "<<Max_B[i][ii]<<endl;
      c12[i]->cd(ii+1);
