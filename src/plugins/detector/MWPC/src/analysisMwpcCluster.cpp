...
Long_t Det_MWPC::histos_cluster(){
  ...
  for(int i=0;i<12;i++){
    ...
    sstr_name<<"Total charge vs multiplicity for "<<"MwpcC"<<i/4+2<<(char)((i%4)/2+88)<<(char)((i%4%2)*6+76);
    sstr_title<<sstr_name.str()<<";Multiplicity;Total charge";
...
Long_t Det_MWPC::done_cluster(){
  ...
  ostringstream sstr_name, sstr_title;
  for(int i=0;i<12;i++){
    sstr_name<<"Statistic of multiplicity for "<<"MwpcC"<<i/4+2<<(char)((i%4)/2+88)<<(char)((i%4%2)*6+76);
    sstr_title<<sstr_name.str()<<";Multiplicity; Composition (%)";
