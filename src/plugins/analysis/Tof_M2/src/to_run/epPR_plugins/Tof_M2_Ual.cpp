...
int Ep_uc = 12;
int Ep_c = 6;
 final_PgCAdc =  new TH1F( "final_PgCAdc","final_PgCAdc",30,10,adc_uc);
...
if(par=="epPR") M = 0.511;
  ...
  string walk_parm =  Form("/data/trek/E36/gautam/crnRoot/cooker/inputFiles/tofM2/walkparm/kmu2_walkparm.dat");
  ...
  int ix =0;
  string inputfile_A=Form("/data/trek/E36/gautam/crnRoot/cooker/inputFiles/tofM2/offset/meanA_kmu2_1.dat");
  ...
  string inputfile_B = Form("/data/trek/E36/gautam/crnRoot/cooker/inputFiles/tofM2/offset/meanB_kmu2_1.dat");
...
Long_t Tof_M2::process()
{
  ...
  if(ptr->nTracks==1 &&  ptr->fVertSP <300. && 
     ptr->fVertSP>30 && ptr->fChi2PerNDF<10 && 
...
if(fTofM2<3000)final_M2->Fill(fTofM2);
      if(fTofM2<3000){
...
Long_t Tof_M2::done()
{
  FILE *mean_B1 = fopen(Form("/data/trek/E36/gautam/crnRoot/cooker/inputFiles/tofM2/pgc_gains/PgC_Gain_align_peak_%s.dat",par.Data()),"w");
  ...
  for(int i = 0; i < 12; i++){
    ...
    for(int ii = 0; ii < 7; ii++){
      ...
      ADC_PgCh[i][ii]->Draw("HIST");
           Max_B[i][ii] =  EP_PgCh[i][ii]->GetBinCenter(EP_PgCh[i][ii]->GetMaximumBin());;
