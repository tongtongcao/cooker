...
void MwpcEvent::addHistData_pedSub(int* value, const double* pdd, const double *sigma){
  for(int i=0;i<numBin;i++){
    if(stripTransStat==1){
      if(value[i]-pdd[i]>0)
        eventHist->SetBinContent(i+1,(value[i]-pdd[i])/sigma[i]*20);
      else
        eventHist->SetBinContent(i+1,0);
    }
    else{
      if(value[C2XStTrans[i]]-pdd[i]>0)
        eventHist->SetBinContent(i+1,(value[C2XStTrans[i]]-pdd[i])/sigma[i]*20);
      else
        eventHist->SetBinContent(i+1,0);
    }
  }
...
double* MwpcEvent::getAdcValue(int* value, const double* pdd,const double *sigma){
  double *adcValue=new double[numBin];
  for(int i=0;i<numBin;i++){
    if(stripTransStat==1){
      if(value[i]-pdd[i]>0)
        adcValue[i]=(value[i]-pdd[i])/sigma[i]*20;
      else
        adcValue[i]=0;
    }
    else{
      if(value[C2XStTrans[i]]-pdd[i]>0)
        adcValue[i]=(value[C2XStTrans[i]]-pdd[i])/sigma[i]*20;
      else
        adcValue[i]=0;
    }
  }
...
vector<vector<int> > MwpcEvent::clusterStripNumber(MwpcEvent* event,int* value, const double* pdd, const double* sigma, bool statusTotalChargeCut, const double* totalChargeCut, bool statusSingletThreshold, double singletThreshold, bool statusDoubletThresholdTotalChargeCut, double doubletThreshold, double doubletTotalChargeCut){
 ...
 // Thrshold cut for singlet and doublet is applied to filter clusters so as to obtain a first-filtered vector 
  double* AdcValue=event->getAdcValue(value, pdd); // Get Adc values after pedestal subtraction 
  double* AdcValueGainCali=event->getAdcValue(value, pdd, sigma, 0); // Get Adc Values after pedestal subtraction and gain calibtration
  ...
  for(int i=0;i<(int)vect_clus.size();i++){
    ...
    if(vect_clus[i].size()==2){
      if(statusDoubletThresholdTotalChargeCut==1){ // Status == 1 -- Apply threshold-total-charge cut for doublet; Status == 0 -- Dont apply
        if((AdcValue[vect_clus[i][0]]<sigma[vect_clus[i][0]]*doubletThreshold || AdcValue[vect_clus[i][1]]<sigma[vect_clus[i][1]]*doubletThreshold) && AdcValueGainCali[vect_clus[i][0]]+AdcValueGainCali[vect_clus[i][1]]<doubletTotalChargeCut) continue; // Cut off if one of two Adc values is less than threshold while total charge is less than total-charge cut value 
        else{
          vect_clus_after_thresholdcut.push_back(vect_clus[i]);
          continue;
        }
  ...
  for(int i=0;i<(int)vect_clus_after_thresholdcut.size();i++){
    if(statusTotalChargeCut==1){ // Status == 1 -- Apply total charge cut; Status == 0 -- Dont apply
      ...
      for(int j=0;j<(int)vect_clus_after_thresholdcut[i].size();j++){
        totalcharge+=AdcValueGainCali[vect_clus_after_thresholdcut[i][j]];
      }
