...
//What does this function do!! bool doDraw??
double peakSelect::getPedestal(bool doDraw){
  ...
  if(f1==0){
    ...
    //get position of the x-peaks
    for(int p = 0; p < nfound; p += 1){
      xpos = xpeaks[p];
    }
    std::sort(xpeaks,xpeaks+nfound);
    lowRange=/*mH1->GetBinLowEdge(mH1->GetMaximumBin()-16);  */xpeaks[0]/2.0;
    upRange=/*mH1->GetBinLowEdge(mH1->GetMaximumBin()+17);  */xpeaks[0]*3.0/2.0;
