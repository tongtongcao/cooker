...
Long_t Tof_M2::histos(){
  int adc_c = 400;
  int Ep_c = 4;
  final_PgCAdc =  new TH1F( "final_PgCAdc","final_PgCAdc",25,0.1,600);
  final_PgCEp =  new TH1F( "final_PgCEp","final_PgCEp",30,0,4);
  ...
  for(int i = 0; i < 12; i++){
    ...
    h1adc_PgC[i] = dH1(name1.c_str(),title1.c_str(),20,5,400);
    ...
    h1EP_PgC[i] = dH1(name1kk.c_str(),title1kk.c_str(),20,0,4);
    ...
    for(int ii=0;ii<7;ii++){
      ...
      ADC_PgCh[i][ii]= dH1(namep.c_str(),titlep.c_str(),10,0,adc_c);
      ...
      titlep11+=(ii)+'0';
      titlep11+="";
      EP_PgCh[i][ii]= dH1(namep11.c_str(),titlep11.c_str(),10,0,Ep_c);
...
Long_t Tof_M2::startup()
{
  ...
  string inputfile_C = Form("/data/trek/E36/gautam/crnRoot/cooker/inputFiles/tofM2/pgc_gains/PgC_Gain_align_peak_%s.dat",par.Data());
	  ...
	  if(Array_ADC[i][0]>cut_val && Array_ADC[i][1]<cut_val && Array_ADC[i][2]<cut_val && Array_ADC[i][3]<cut_val && Array_ADC[i][4]<cut_val && Array_ADC[i][5]<cut_val && Array_ADC[i][6]<cut_val)  
	    {
	      ...
	      // cout<<" i "<<i<<" j "<<0<<"  ADC " <<array_ADC[i][0]<<endl;
