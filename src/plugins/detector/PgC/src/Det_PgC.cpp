...
Long_t Det_PgC::histos()
{
  for(int i=0;i<12;i++){
    ...
    title1E+=(i+1)%10+'0';
    title1E+="Ene";
    h1E_PgC[i] = dH1(Form("PgC_Gap_KE%d",i+1),Form("PgC_Gap_KE%d",i+1),50,0,400);
    ...
    h1Mom_PgC[i] = dH1(Form("PgC_Mom_Gap_%d ",i+1),Form("PgC_Mom_Gap_%d ",i+1),100,0,260);
...
Long_t Det_PgC::process() 
{
 ...
 Int_t nTracks = ptr->nTracks;  
cout<<nTracks<<endl;
if(nTracks==1 && 0<ptr->fChi2PerNDF && ptr->fChi2PerNDF<5.&& ptr->fFlagToFMatch==1 && (ptr->fFlagTargetToAC==0 || ptr->fFlagTargetToAC==1) && GapNum>-1. && GapNum<13.)
{
//Tof2 
...
E_loss_degb[GapNum] = (ptr->fTof2SP - ploss_tof_b[GapNum] - ploss_deg_f[GapNum])/sqrt((ptr->fTof2SP - ploss_tof_b[GapNum] - ploss_deg_f[GapNum])*(ptr->fTof2SP -ploss_tof_b[GapNum]- ploss_deg_f[GapNum]) + M*M)*ploss_deg_b[GapNum];
  ...
  h1E_PgC[GapNum]->Fill(E_PgC[GapNum]-M);
