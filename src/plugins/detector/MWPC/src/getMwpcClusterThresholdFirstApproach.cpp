...
#include "MwpcEventThresholdSecondApproach.h"
using namespace std;
...
Long_t Det_MWPC::done_clusterthresholdFirstApproach(){
  ...
  for(int i=0;i<12;i++){
    for(int k=0;k<nType;k++){
      sstr_title<<"CPST vs factor for "<<"MwpcC"<<i/4+2<<(char)((i%4)/2+88)<<(char)((i%4%2)*6+76);
      ...
      gr[k][i]->GetYaxis()->SetTitle("Composition (%)");
  ...
  TCanvas *canv=new TCanvas("Composition vs NSigma for clusters","Composition vs NSigma for clusters",2000,1500);
  canv->Divide(4,3);
  for(int i=0;i<12;i++){
    canv->cd(i+1);
    for(int k=0;k<nType;k++){
  ...
  ostringstream sstr;
  sstr<<"Composition vs "<<nPoint*0.5<<"Sigma for clusters.pdf";
  ...
  TCanvas *canvSingMini=new TCanvas("Composition vs NSigma for Singlet for Cluster","Composition vs NSigma for Singlet for Cluster",2000,1500);
  ...
  sstr<<"Composition vs "<<"Sigma for Singlet for First Approach for Cluster.pdf";
  ...
  TCanvas *canvDoubMini=new TCanvas("Composition vs NSigma for Doublet for Cluster","Composition vs NSigma for Doublet for Cluster",2000,1500);
  ...
  sstr<<"Composition vs "<<"Sigma for Doublet for First Approach for Cluster.pdf";
