...
int adc_c = 400;
int Ep_c = 4;
  final_PgCAdc =  new TH1F( "final_PgCAdc","final_PgCAdc",22,0,500);
...
Long_t Tof_M2::process()
{
  ...
  if(ptr->nTracks==1 &&  ptr->fVertSP <300. && 
	...
	h1Mom_PgC[ptr->fgapNumTof2]->Fill(tof2_mom[ptr->fgapNumTof2]);
             ...
             int cut_val =70;
	for(int i=0;i<12;i++){
	  ADC_PgC[i]=0; ADC_PgCf=0;Ep_PgCf=0;
	  int count_j=0;
	  for(int ii=0;ii<7;ii++){
...
count_j++;
  ...
  if(ADC_PgCf>20) final_PgCAdc->Fill(ADC_PgCf);
if(Ep_PgCf>0.2) final_PgCEp->Fill(Ep_PgCf);
